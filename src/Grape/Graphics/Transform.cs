...
/// matrix, exposed through <see cref="IRenderArgs{TSelf}"/> so
...
public struct Transform : IRenderArgs<Transform>
{
    ...
    /// </summary>
    public static implicit operator Transform(Matrix4x4 matrix) =>
        new Transform(matrix);

    /// <inheritdoc cref="IRenderArgs{TSelf}.GetTransform"/>
    public static Func<Transform, Matrix4x4>? GetTransform { get; } =
        args => args.Matrix;

    /// <inheritdoc cref="IRenderArgs{TSelf}.SetTransform"/>
    public static Func<Transform, Matrix4x4, Transform>? SetTransform { get; } =
        (args, m) => new Transform(m);
}
